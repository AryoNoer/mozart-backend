...
def add_data():
    try:
        new_data = request.json.get("user")
...
def get_user(user_id):
    try:
        ...
        app.logger.error(f"Error in GET user request: {str(e)}")
        return jsonify({"error": "Internal Server Error"}), 500
...
def update_user(user_id):
    try:
        ...
        app.logger.error(f"Error in PUT user request: {str(e)}")
        return jsonify({"error": "Internal Server Error"}), 500
...
def delete_user(user_id):
    try:
        ...
        app.logger.error(f"Error in DELETE user request: {str(e)}")
        return jsonify({"error": "Internal Server Error"}), 500
